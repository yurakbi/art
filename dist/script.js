  ...
  btns.forEach(btn => {
    btn.addEventListener('click', function () {
      ...
      if (this.classList.contains('active-style')) {
        this.nextElementSibling.style.maxHeight = this.nextElementSibling.scrollHeigth + 80 + "px";
      } else {
  ...
  function createMask(event) {
    ...
    if (event.type === 'blur') {
      if (this.value.length == 2) {
        ...
        setCursorPosition(this.value.length, length);
      }
    }
  }
...
const scrolling = upSelector => {
  ...
  let links = document.querySelector('[href^="#"]'),
      speed = 0.3;
  links.forEach(link => {
    link.addEventListener('click', function (event) {
      ...
      function step(time) {
        ...
        r = toBlock < 0 ? Math.max(withTop - progress / speed, widthTop + toBlock) : Math.min(withTop + progress / speed, widthTop + toBlock);
  ...
  //     body = document.body;
  // const calcScroll = () => {
  //     upElem.addEventListener('clock', function(event) {
